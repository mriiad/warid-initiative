...
export function fromNameLabels<T extends string>(
	src: Array<[T, string]>
): Array<LabelledEnum<T>> {
	return src.map(([name, label]) => ({ name, label }));
}

function fromLabelledEnum<T extends string>(
	src: Array<LabelledEnum<T>>
): { [K in T]: K } {
...
export const GenderValues = fromNameLabels([
	['male', 'Male'],
	['female', 'Female'],
]);
export const GenderType = fromLabelledEnum(GenderValues);
